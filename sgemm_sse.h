...
static __declspec(align(16)) float _a[MC*KC];
static __declspec(align(16)) float _b[KC*NC];
static __declspec(align(16)) float _c[MR*NR];
...
static inline void dot8x8_avx(const float *a, const float *b, float *c, const int kc)
{
	...
	_mm_prefetch( (const char *)b, 2 );
	_mm_prefetch( (const char *)a, 0 );
...
{
	static __declspec(align(16)) float AB[MR*NR];
	int i, j;
